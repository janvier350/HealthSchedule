<div class="app-header__logo">
                    <div class="logo-src"></div>
                    <div class="header__pane ml-auto">
                        <div>
                            <button type="button" class="hamburger close-sidebar-btn hamburger--elastic" data-class="closed-sidebar">
                                <span class="hamburger-box">
                                    <span class="hamburger-inner"></span>
                                </span>
                            </button>
                        </div>
                    </div>
    </div>
    <div class="app-header__mobile-menu">
                    <div>
                        <button type="button" class="hamburger hamburger--elastic mobile-toggle-nav">
                            <span class="hamburger-box">
                                <span class="hamburger-inner"></span>
                            </span>
                        </button>
                    </div>
    </div>
    <div class="app-header__menu">
                    <span>
                        <button type="button" class="btn-icon btn-icon-only btn btn-primary btn-sm mobile-toggle-header-nav">
                            <span class="btn-icon-wrapper">
                                <i class="fa fa-ellipsis-v fa-w-6"></i>
                            </span>
                        </button>
                    </span>
    </div>
<div class="scrollbar-sidebar">
    <div class="app-sidebar__inner">
        <ul class="vertical-nav-menu">
            <li class="app-sidebar__heading">Dashboards</li>
            <li>
                <a href="home_vta.php" class="mm-active">
                    <i class="metismenu-icon pe-7s-rocket"></i>
                    Estadisticas
                </a>
            </li>
            <li class="app-sidebar__heading">Panel de Control</li>
            <li>
                <a href="#">
                    <i class="metismenu-icon pe-7s-add-user"></i>
                    Clientes
                    <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                </a>
                <ul>
                    <li>
                        <a href="PNC_ClienteCrear.php">
                            <i class="metismenu-icon"></i>
                            Crear Cliente
                        </a>
                    </li>
                    <li>
                        <a href="PNC_ClienteListado.php">
                            <i class="metismenu-icon">
                            </i>Listado
                        </a>
                    </li>
                    
                </ul>
            </li>
        
            <li class="app-sidebar__heading">Ventas</li>
            <li>
                <a href="VTA_Registro.php">
                    <i class="metismenu-icon pe-7s-display2"></i>
                    Crear Nueva
                </a>
            </li>
            <li>
                <a href="VTA_Vigente.php">
                    <i class="metismenu-icon pe-7s-date"></i>
                    En Proceso
                </a>
            </li>
            <li>
                <a href="VTA_Prioridad.php">
                    <i class="metismenu-icon pe-7s-date"></i>
                    Prioridad
                </a>
            </li>                
            <li>
                <a href="VTA_Concretado.php">
                    <i class="metismenu-icon pe-7s-diamond"></i>
                    Concretadas
                </a>
            </li>
            <li>
                <a href="VTA_Negado.php">
                    <i class="metismenu-icon pe-7s-back-2"></i>
                    Negadas
                </a>
            </li>

            
            <li class="app-sidebar__heading">Reportes</li>        
            <li>
                <a href="RPT_Vendedor_Vta.php">
                    <i class="metismenu-icon pe-7s-graph"></i>
                    Mis Ventas
                </a>
            </li>

            
        </ul>
    </div>
</div>